...
from sqlalchemy import select, text  # ✅ Thêm text import
...
# ✅ FIX: Đọc từ environment, dùng postgres hostname thay vì localhost
...
async def session(test_engine: AsyncEngine) -> AsyncGenerator[AsyncSession, None]:
    ...
    async with async_session() as session:
        ...
        # ✅ FIX: Clean up data sau mỗi test để tránh conflict
        try:
            await session.execute(text("TRUNCATE TABLE traces, molecules, runs CASCADE"))
...
class TestRunModel:
    ...
    async def test_query_run_by_status(self, session: AsyncSession):
        ...
        # ✅ FIX: Filter by IDs của test này
        test_ids = [run1.id, run2.id, run3.id]
...
class TestTraceModel:
    ...
    async def test_query_traces_by_round(self, session: AsyncSession):
        ...
        traces = [
            ...
            for i, round_num in enumerate([1, 1, 2, 3])
        ]
